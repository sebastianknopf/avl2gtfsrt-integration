...
class AvlDataInstance:
    ...
    def _run_internal(self) -> None:
        ...
        # check which adapter type (polling/realtime) we have here ...
        if isinstance(self._adapter, PollingAdapter):
            
            # this is a polling adapter, so we need to manage its lifecycle by intervals
            # after each cycle, wait for the configured timespan until the next request
